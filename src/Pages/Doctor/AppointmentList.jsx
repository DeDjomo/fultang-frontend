...
export function AppointmentList() {
    ...
    const filteredAppointments = appointmentList.filter((appointment) => {
        ...
        if (filter === "upcoming") {
            // Rendez-vous à venir: en_attente ET date future
    ...
    async function retrieveDoctorAppointments(doctorId) {
        ...
        try {
            ...
            setIsLoading(false);
            if (response.success) {
                // La réponse contient { success: true, count: X, data: [...] }
    ...
    return (
        <CustomDashboard linkList={doctorNavLink} requiredRole={"Doctor"}>
            ...
            <div className="mx-auto p-6 h-fit">
                <h1 className="text-3xl font-bold text-gray-800 mb-6">{`Appointments of Dr. ${(userData?.prenom || "") + " " + (userData?.nom || "")}`}</h1>
