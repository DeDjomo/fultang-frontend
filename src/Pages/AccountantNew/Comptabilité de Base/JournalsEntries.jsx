...
import { sampleChartOfAccounts, sampleEntries } from "../Data/mockFinancialData";
...
export function JournalEntries() {
    ...
    // sampleEntries removed - imported from Data/mockFinancialData
    ...
    const loadJournalEntries = useCallback(async () => {
        ...
        try {
            ...
            if (storedEntries) {
                setEntriesList(JSON.parse(storedEntries));
            } else {
                // Initial seed
                setEntriesList(sampleEntries);
                localStorage.setItem('financialEntries', JSON.stringify(sampleEntries));
            }
            setErrorStatus(null);
        } catch (error) {
            console.error(error);
    ...
    const handleSaveEntry = async (entryData, isValidationIntent) => {
        ...
        if (editingEntry && editingEntry.id) {
            updatedList = entriesList.map(e => e.id === finalEntryData.id ? finalEntryData : e);
        } else {
            updatedList = [{ ...finalEntryData, id: uuidv4() }, ...entriesList];
        }

        setEntriesList(updatedList);
        localStorage.setItem('financialEntries', JSON.stringify(updatedList));
    ...
    const handleDeleteEntry = async (entryId) => {
        if (confirm("Êtes-vous sûr de vouloir supprimer cette écriture (si brouillon) ?")) {
            ...
            if (entryToDelete && entryToDelete.status === "Brouillon") {
                setIsLoading(true);
                const updatedList = entriesList.filter(e => e.id !== entryId);
                setEntriesList(updatedList);
                localStorage.setItem('financialEntries', JSON.stringify(updatedList));
                setIsLoading(false);
            } else {
                    ...
                    <div className="flex-grow overflow-y-auto pr-2 space-y-6">
                        ...
                        <div className="bg-gradient-to-r from-teal-50 to-cyan-50 p-6 rounded-xl border border-teal-200">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Nom du Patient <span className="text-gray-400 text-xs">(Optionnel)</span></label>
                                    <input type="text" placeholder="Ex: Jean Dupont" value={formData.patientName} onChange={(e) => handleInputChange(e, 'patientName')} disabled={!canEdit}
                                        className={`w-full px-4 py-3 border border-gray-300 rounded-xl ${themeColors.focusRing} transition-all ${!canEdit ? 'bg-gray-100' : 'hover:border-teal-400'}`} />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Catégorie de Recette <span className="text-gray-400 text-xs">(Pour ventilation)</span></label>
                                    <select value={formData.revenueCategory} onChange={(e) => handleInputChange(e, 'revenueCategory')} disabled={!canEdit}
                                        className={`w-full px-4 py-3 border border-gray-300 rounded-xl ${themeColors.focusRing} transition-all ${!canEdit ? 'bg-gray-100' : 'hover:border-teal-400'}`}>
                                        ...
                                        {filteredEntries.map((entry, index) => (
                                            <tr key={entry.id} className="hover:bg-gradient-to-r hover:from-teal-50 hover:to-cyan-50 transition-all duration-300 group">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-center">
                                                    <span className={`px-4 py-2 inline-flex text-sm leading-5 font-semibold rounded-full ${entry.status === 'Validée' ? 'bg-green-100 text-green-800' :
